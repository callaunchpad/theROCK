...
def find_element_attr(browser, tag_name, attribute):
    ...
    values = None
    try:
        elems = browser.find_elements('tag name', tag_name)
...
def click_holdsetup(browser, holdsetup='MoonBoard 2016'):
    ...
    elems = browser.find_elements('tag name', 'select')
    for e in elems:
    ...
    elems = target_elem.find_elements('tag name', 'option')
    target_elem = None
    for e in elems:
        if e.text == holdsetup:
            target_elem = e
...
def get_current_page(browser):
    ...
    elems = browser.find_elements('tag name', 'div')
    for e in elems:
    ...
    page_elems = pager_elem.find_elements('tag name', 'span')
    for e in page_elems:
...
def get_problems(browser):
    ...
    problems = []
    data_ids = []
    elems = browser.find_elements('tag name', 'tr')
...
def get_problem_meta(problem):
    ...
    h3 = problem.find_elements('tag name', 'h3')[0]
    meta['problem_name'] = h3.text
    meta['info'] = [p.text for p in problem.find_elements('tag name', 'p')]
    meta['url'] = h3.find_elements('tag name', 'a')[0].get_attribute('href')

    # Rating information
    stars = [star.get_attribute('src') for star in problem.find_elements('tag name', 'img')]
...
def click_next_repeats(browser, current_page):
    ...
    spans = browser.find_elements('tag name', 'span')
    for s in spans:
    ...
    items = browser.find_elements('tag name', 'a')
    for i in items:
        ...
        for x in range(num_tries):
            try:
                ...
                for repeat in repeats:
                    h3_item = repeat.find_elements('tag name', 'h3')[0]
                    user_name = h3_item.text
                    uid = h3_item.find_elements('tag name', 'a')[0].get_attribute('href').split('/Account/Profile/')[1]

                    repeats_add = [r.text for r in repeat.find_elements('tag name', 'p')] + [user_name, uid]
